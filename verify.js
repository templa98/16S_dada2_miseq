...
function validateConfig(config) {
  ...
  if (!config.settings) {
    ...
    if ('run_experiment' in settings && !isBoolean(settings.run_experiment)) {
      ...
      // return and don't verify the configs if the experiment is not running
      if(!settings.run_experiment){
        console.log("Verification skipped for this experiemnt as 'run_experiment' is set to 'false'.")
        return [];
      }
    }
    ...
    if (!('pipeline' in settings)) {
      ...
      // check for quality control pipeline
      ...
      //check for dada2 pipeline
      if (!isBoolean(pipeline.dada)) {
  ...
  if (pipeline_stages.qc) {
    if (!config.quality_control) {
      ...
      const requiredQcKeys = ['quality_profile_plot']; // dropped support for 'rarefaction_curve', 'multiqc' 
      ...
      // TODO: fix multiqc on compute canada and 
  ...
  // validate dada2 pipeline
  if (pipeline_stages.dada){
    if(!config.dada){
      errors.push('Missing "dada" pipeline section.');
    } else{
      // Validate "filter_and_trim" section
      if (!config.dada.filter_and_trim) {
        errors.push('Missing "filter_and_trim" section.');
      } else {
        const ft = config.dada.filter_and_trim;
      ...
      if (!config.dada.asv_inference) {
        errors.push('Missing "asv_inference" section.');
      } else {
        const asv = config.dada.asv_inference;

        // Check for required keys in "asv_inference"
        const requiredAsvKeys = ['error_model', 'dada_pool_samples', 'generate_fasta_file'];
        ...
        if ('generate_fasta_file' in asv && !isBoolean(asv.generate_fasta_file)) {
          errors.push('"asv_inference.generate_fasta_file" must be a boolean.');
        }
      }

      // Validate "taxonomy_assignment" section
      if (!('taxonomy_assignment' in config.dada)) {
        errors.push('Missing "taxonomy_assignment" section.');
      } else if (!Array.isArray(config.dada.taxonomy_assignment)) {
        errors.push('"taxonomy_assignment" must be an array.');
      } else {
        config.dada.taxonomy_assignment.forEach((ta, index) => {
